...
class CpNltNetDEQModule(DEQModule):
    ...
    def forward(self, z1s, us, **kwargs):
        ...
        z1s_out = RootFind.apply(self.func, z1s, us, threshold, train_step)
        if self.training:
            z1s_out = RootFind.f(self.func, z1s_out, us, threshold, train_step)
            z1s_out = self.Backward.apply(self.func_copy, z1s_out, us, threshold, train_step)
        return z1s_out
